  ...
  const handleReloadCache = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        alert(`Sukces: ${data.message}\nScenariusze: ${data.scenarios_count}\nZestawy: ${data.bundles_count}`);
        window.location.reload();
      } else {
        const data = await response.json().catch(() => ({}));
        alert(`Błąd odświeżania pamięci podręcznej: ${data.error || 'Nieznany błąd'}`);
      }
    } catch (err) {
      alert(`Błąd sieci podczas odświeżania pamięci podręcznej: ${err.message}`);
    }
  };
  ...
  return (
    <header className={styles.header}>
      <div className={styles.brand}>
        ...
        <span className={styles.tagline}>Interaktywne Laboratorium Kubernetes</span>
      </div>
      ...
      <div className={styles.right}>
        ...
        <a
          ...
          title="Odwiedź repozytorium GitHub"
        ...
        <div className={`${styles.clusterBadge} ${clusterReady ? styles.ready : styles.notReady}`}>
          ...
          <span>{clusterReady ? 'Klaster Gotowy' : 'Łączenie…'}</span>
        </div>
        ...
        <div className={styles.reloadBtnContainer}>
          <button
            ...
            aria-label="Odśwież pamięć podręczną scenariuszy"
          >
